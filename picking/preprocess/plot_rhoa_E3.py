...
hourly_avg, daily_rainfall = read_hydro_data(r'水文站資料彙整_20240731.xlsx')
...
def plot_hydro_data(hourly_avg, daily_rainfall, plot_target,dates):
    ...
    ax3.plot(dates,median_RHOA, 'bo',markersize=1,label='Median resistivity')
    ax3.set_ylim([0,80])
    ax3.set_ylabel('Median resistivity (Ohm m)', color='b')  # Set label for the secondary Y-axis

    ax4 = ax1.twinx()
    ax4.spines['right'].set_position(('outward', 120))  # Move the third axis outward
    ax4.plot(hourly_avg.index, hourly_avg['Suction_50cm'],linewidth=1,linestyle='--',color='r', label='Suction_50cm')
    ax4.plot(hourly_avg.index, hourly_avg['Suction_100cm'],linewidth=1,linestyle='--',color='y', label='Suction_100cm')
    ax4.set_ylim([0,0.15])
    ax4.set_ylabel('Suction (m)', color='k')  # Set label for the secondary Y-axis

    fig.legend(loc='upper right',bbox_to_anchor=(0.99,0.99))  # Adjust legend position
        

    return fig

# plot_target = ['10cm', '20cm', '30cm',
#     '40cm', '60cm', '80cm',
#     '100cm', '150cm',
#     '200cm',
#        '300cm', '400cm', '500cm',

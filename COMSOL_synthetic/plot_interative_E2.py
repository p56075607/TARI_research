...
linename='E2'
pkl_path = r'D:\R2MSDATA\TARI_E2_test'
...
for i, pkl_file in enumerate(pkl_files):
    with open(join(pkl_path, pkl_file), 'rb') as f:
        if i == 0:
            dates_E2 = pickle.load(f)
            median_RHOA_E2 = pickle.load(f)
        else:
            dates_E2 += pickle.load(f)
            median_RHOA_E2 += pickle.load(f)

# %%
# Save the dates and median_RHOA_E2 to a pickle file
...
median_rhoa_plot, = ax.plot(dates_E2, median_RHOA_E2, 'ro',markersize=3 ,zorder=2)
...
begin_index = 1#dates_E2.index(datetime(2024, 3, 18, 1, 0))
end_index = -1#dates_E2.index(datetime(2024, 6, 6, 21, 0))
ax.set_xlim(dates_E2[begin_index], dates_E2[end_index])
...
def onclick(event):
    if event.inaxes == ax:  # 確保點擊發生在 ax 上
        ...
        threshold_x = 0.1
        threshold_y = 0.1
        for x, y in zip(mdates.date2num(dates_E2), median_RHOA_E2):
...
save_folder = r'D:\R2MSDATA\TARI_E2_test\timelapsed_resistivity_difference'
...
X_index = np.where((X[0] >= 20) & (X[0] <= 40))[0]
...
save_folder = r'D:\R2MSDATA\TARI_E2_test\timelapsed_resistivity_difference'
plot_timeseries(median_RHOA_E2, daily_rainfall, dates_E2, begin_index-2, end_index,current_index)

...
# plt.show()
# %%
# # Use apparent resistivity to calculate water content
filterd_hydro_data['SWC_rhoa'] = log_model_inverse(filterd_hydro_data['rhoa'], a, b)

fig, ax = plt.subplots(figsize=(15, 8))
# Plot the filterd_hydro_data['mean_1m'] at ax2
filterd_hydro_data['date_time'] = pd.to_datetime(filterd_hydro_data['date_time'])
# index = filterd_hydro_data['date_time'] == datetime(2024, 10, 8, 8, 0)
index = filterd_hydro_data['date_time'] == datetime(2024, 10, 14, 14, 0)
# index = filterd_hydro_data['date_time'] == datetime(2024, 10, 31, 16, 0)
filterd_hydro_data['SWChange_rhoa'] = (filterd_hydro_data['SWC_rhoa']-list(filterd_hydro_data['SWC_rhoa'][index])[0])/100

ax.plot(filterd_hydro_data['date_time'], filterd_hydro_data['SWChange_rhoa'], '-o',color='k', linewidth=3, markersize=10, label=r'$\rho_a$ at x=18~19 m')


filterd_hydro_data['SWChange_hydro'] = (filterd_hydro_data['mean_1m']-list(filterd_hydro_data['mean_1m'][index])[0])/100
ax.plot(filterd_hydro_data['date_time'], filterd_hydro_data['SWChange_hydro'], 
        '-or', linewidth=3, markersize=10, label='Contact Sensor')
# index = df.index == datetime(2024, 10, 14, 14, 0)
# ax.plot(df.index, (df["mean_1m"]-list(df['mean_1m'][index])[0])/100, 
#         '-or', linewidth=3, markersize=10, label='Contact Sensor')
ax.legend(fontsize=20)


# ax.set_xlim([datetime(2024, 10, 8, 0, 0), datetime(2024, 10, 14, 16, 0)])
ax.set_xlim([datetime(2024, 10, 14, 10, 0), datetime(2024, 10, 22, 0, 0)])
# ax.set_xlim([datetime(2024, 10, 31, 10, 0), datetime(2024, 11, 7, 0, 0)])
...
start_time = pd.Timestamp('2024-10-19 04:00')
end_time = pd.Timestamp('2024-10-22 00:00')
